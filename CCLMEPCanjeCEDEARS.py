    ...
    for ticker in tickers:
        try:
            ...
            if 'Close' not in hist.columns or 'Volume' not in hist.columns:
                st.warning(f"Missing expected columns for {ticker}")
                continue

            # Fetching latest data
            latest_price = hist['Close'].iloc[-1] if not hist['Close'].empty else None
            latest_volume = hist['Volume'].iloc[-1] if not hist['Volume'].empty else None

            if latest_price is not None and latest_volume is not None and latest_volume >= 1:
                data[ticker] = {
                    "price": latest_price,
                    "volume": latest_volume
                }
            else:
                st.warning(f"Data for {ticker} is incomplete or volume is too low")
...
def get_required_tickers(option):
    ...
    return df['CEDEAR-ARS'].unique()

if st.button("Enter"):
    option = st.selectbox("Select an option:", ["Option1", "Option2"])  # Adjust as needed
    ...
    if not latest_data:
        ...
        for column in ["CEDEAR-ARS", "CEDEARD", "Subyacente"]:
            ...
            for ticker in unique_tickers:
                if ticker in latest_data:
                    prices[ticker] = latest_data[ticker]["price"]
                    volumes[ticker] = latest_data[ticker]["volume"]
        ...
        if df.empty:
            st.error("No valid rows found in the DataFrame after filtering.")
        else:
            ...
            if df.empty:
                ...
                fig_canje = px.scatter(filtered_df_canje, x='X_Canje', y='Y_Canje', hover_name='CEDEAR-ARS', title='CANJE CEDEARS')
